...
| OQ7 | GitHub branch protection on `main` is now **available** (repo public per OQ2). Configuring it meaningfully (required PR + the 5 CI checks) would change the current per-step *direct-to-main* commit workflow — so the branch-protection posture is an operator workflow decision (keep direct-to-main / lightweight force-push+deletion guard / full PR-gated). | Operator decision |
