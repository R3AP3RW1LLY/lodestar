...
- Status: [ ] TODO
- Zone: GREEN
- Files: `packages/voice/src/tts/piper.ts` (the supervisor-driven Piper client), `apps/desktop/src/main/tts-service.ts`, Settings TTS section (test-phrase button, voice/volume) + tests
- Work: first-run download of the pinned Piper release + one voice model via the Step 0.10 artifact downloader (committed in-repo SHA-256 provenance manifest — closes the 0.10 note) into `%APPDATA%/lodestar/voices/`; the Piper client drives the binary through the 2.7a supervisor (text→audio), CPU only (no GPU flags). Wire the speech queue's `speak` hook to the 2.6 Assay `verdict` callouts ("Platinum thirty-two percent — mine"); settings toggle, voice pick, volume, test-phrase button.
- Acceptance criteria:
  - Piper client integration-tested against the real Piper binary (synthesizes a WAV); the download verifies the committed SHA-256 (mismatch refuses).
  - Manual audio check via the Settings test-phrase button, documented.
- Verify by: `pnpm --filter @lodestar/voice test -- piper` + manual: Settings test phrase audible
