...
| OQ7 | GitHub branch protection on `main` (require all 5 CI check names) is blocked by GitHub's private-repo plan gate; enable when the repo goes public (OQ2) or on a paid plan. Until then, green CI is enforced but not merge-blocking. | Before Phase 11 |
