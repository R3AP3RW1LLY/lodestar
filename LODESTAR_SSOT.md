...
| `http_cache` | egress response cache (key=method+url, body, ETag/Last-Modified, stored-at + TTL); **owned by the Step-4.6 cache store via `CREATE TABLE IF NOT EXISTS`** — ephemeral, regenerable infra OUTSIDE the forward-only migration chain (the `schema_migrations` precedent), never migrated/preserved | 4 |
...
**Migration number registry (forward-only, one owner step each):** 001 init (0.6) · 002 sessions (1.8) · 003 prospects (2.1) · 004 market (2.5; the commodity dictionary is the pure `@lodestar/shared` module from 2.2, not a table) · 005 personal_bests (3.4) · 006 galaxy + market extensions (4.1) · 007 alert_rules (4.11) · 008 chat_messages (5.5) · 009 ml_models (6.3) · 010 calibration_weights (6.5) · 011 carrier tables (8.1) · 012 wing tables (9.2) · 013 community_submissions (10.3). New tables enter this registry before their migration is written.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `apps/desktop/src/renderer/screens/{CommandDeck.tsx,deck-status.ts}`, `components/{SituationPanel,CargoPanel,SessionStatsPanel,FuelPips,VesselStrip,Stat}.tsx`, `hooks/use-now.ts`, `format.ts` + tests, `docs/verification/phase-1.md`, `apps/desktop/scripts/replay-journal.mjs`. **UI REDESIGN (2026-07-13, changelog-noted):** the original six equal `{Ship,Location,Fuel,Cargo,Activity,Session}` tiles were reorganized into a hero bento — `SituationPanel` (activity + location + flags), `CargoPanel` (fill gauge vs capacity), `SessionStatsPanel` (rate heroes), `FuelPips` (the single fuel readout), `VesselStrip` (slim ship-identity footer). Same live telemetry + offline/not-configured states; `{ShipPanel,LocationPanel,ActivityPanel}` folded away.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/core/src/journal/events/prospected-asteroid.ts` (Prospect observation + `toProspect` mapper), `packages/data/src/migrations/003-prospects.ts` (inlined SQL const, 001 pattern), `packages/core/src/session/prospect-repository.ts` + tests. (`ProspectedAsteroid`/`AsteroidCracked` are already parsed full-fidelity in `journal/events/parse.ts` from Step 1.5; the crack "linkage" is `markLastCracked` on the repo — no separate `asteroid-cracked.ts` file.)
...
- Work: pure function `assay(prospect, method, thresholds, priceBook) → Verdict` where `Verdict = {call: MINE|SKIP, score, reasons: Reason[]}` — reasons are structured (`proportion-above-threshold`, `motherlode`, `price-weighted-value/t`, `content-tier`, `already-depleted`) so UI and TTS render them verbatim. Value/t computed against best known price via canonical ids. **Precedence is explicit: depleted (`Remaining ≤ 0`) → SKIP beats motherlode; otherwise motherlode → MINE.** Multi-material MINE leads with the **dominant-VALUE** qualifier (price × proportion), proportion as tiebreaker.
- Acceptance criteria:
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/core/src/market/price-book.ts`, `packages/data/src/migrations/004-market.ts` (inlined SQL per the 001 pattern — no `.sql` loader) + tests
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/core/src/assay/orchestrator.ts` + tests (+ `prospect-repository.ts` gained `saveVerdict`/`markActedOn`/`markCracked`)
- Work: the runtime glue that makes Assay real (legal under §3.2 since `core` → `intelligence`): subscribes to prospect events on the bus → merges thresholds + overrides → calls the pure verdict engine with the live price book → persists the verdict + reasons onto the prospect row → emits the `verdict` event on the bus (the main process forwards it to IPC/WS with its renderer consumer — the Assay UI panel, 2.9; the `assay.verdict` channel + engine→bus wiring land there per the "channel added with its consumer" convention) → hands the callout to the speech queue (2.7, once present) → computes the **acted-on flag** (a `MiningRefined`/`AsteroidCracked` of the verdict commodity within the following window marks the prospect acted-on).
- Acceptance criteria:
  ...
  - Acted-on correlation pinned by fixtures (mined-after-MINE, ignored-SKIP); pipeline never blocks the bus (decoupled via the bus, error-isolated, tested).
- Verify by: `pnpm --filter @lodestar/core test -- orchestrator`

#### Step 2.7 — Piper TTS callouts (CPU) + sidecar supervisor
> **DECOMPOSITION (2026-07-13, changelog-noted):** this step is large (a shared supervisor, a priority queue, a real-binary download with committed provenance, main-process wiring, a Settings section, and a manual audio check), so it is split into **2.7a** (the pure, fully-testable infrastructure) and **2.7b** (the real Piper binary + wiring + Settings + manual audio). Each is atomic, green, and independently reviewed — same pattern as 1.9/1.9a.

##### Step 2.7a — speech queue + shared sidecar supervisor
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/voice/src/tts/speech-queue.ts`, `packages/core/src/sidecar/supervisor.ts` + tests (scaffolds `@lodestar/voice`)
- Work: the **shared sidecar supervisor** (spawn, liveness, crash-restart-with-backoff + crash-loop protection, ordered SIGTERM→grace→SIGKILL shutdown — reused by ML/STT in Phases 6–7; all process I/O behind an injected `SpawnSidecar`). Speech queue with **named priority classes — `ops-echo > safety > alert > verdict > ambient` — where a higher class preempts lower-class backlog** (Phase 7's confirmation echo depends on this), dedupe, cancel-on-superseded-verdict. Pure logic + injected async speaker. CPU only.
- Acceptance criteria:
  - Queue semantics fully unit-tested (priority preemption across all five classes, dedupe, cancel-on-superseded-verdict); supervisor lifecycle (spawn/health/crash-restart/shutdown order) tested with a scripted test double.
- Verify by: `pnpm --filter @lodestar/voice test && pnpm --filter @lodestar/core test -- supervisor`

##### Step 2.7b — Piper binary + TTS service + Settings + manual audio
> **DECOMPOSITION (2026-07-13, changelog-noted):** 2.7b spans a redirect-following downloader change, real cross-CDN provenance, two archive formats, a Piper client, main-process audio wiring, and a Settings section, so it is split into **2.7b-i** (verified download + extraction + synthesis — pure/testable + a real-binary integration test) and **2.7b-ii** (TTS service + audio playback + Settings + wire-to-verdicts + the manual audio check). Same 1.9/1.9a pattern.

###### Step 2.7b-i — verified Piper download + extraction + synthesis
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/integrations/src/downloader/artifact-downloader.ts` (redirect-following), `packages/voice/src/tts/{piper-assets.ts,archive.ts,piper.ts}` + tests
- Work: the install downloader now follows redirects with a per-hop INSTALL-allowlist re-check (real GitHub/HF assets 302 to signed, expiring CDN URLs; the committed SHA-256 on the final bytes is the integrity guarantee); added `release-assets.githubusercontent.com` + `cas-bridge.xethub.hf.co` to the install allowlist. `piper-assets.ts`: the committed provenance manifest (Piper Windows binary zip + the `en_US-ryan-high` voice tar.gz, both from GitHub releases with verified SHA-256 — closes the 0.10 note). `archive.ts`: pure-JS zip + tar.gz extraction (`fflate` + a minimal USTAR reader). `piper.ts`: `ensureInstalled` (download+extract on first run) + `synthesize` (one-shot text→WAV) + the injectable node spawn adapter.
- Acceptance criteria:
  - Download verifies the committed SHA-256 (mismatch refuses); redirects are followed only to allowlisted hops (off-allowlist fails closed). Piper synthesizes a real WAV (integration-tested against the real binary, gated on its presence).
- Verify by: `pnpm --filter @lodestar/voice test && pnpm --filter @lodestar/integrations test -- artifact-downloader`

###### Step 2.7b-ii — TTS service + audio playback + Settings + manual audio
- Status: [x] DONE
- Zone: GREEN
- Files: `apps/desktop/src/main/tts-service.ts`, renderer audio playback, Settings TTS section (test-phrase button, voice/volume) + tests
- Work: the desktop main wraps the integrations downloader as the Piper `ArtifactFetcher`, keeps a warm synthesized WAV cache, and on a bus `verdict` (2.6) synthesizes the callout ("Platinum thirty-two percent — mine") and pushes the WAV to the renderer, which plays it (volume-controlled). Settings TTS section: enable toggle, voice pick (default `en_US-ryan-high`), volume, test-phrase button.
- Acceptance criteria:
  - Test-phrase button produces audible speech (manual check, documented); verdict callouts speak end-to-end on a fixture replay.
- Verify by: manual: Settings test phrase audible + fixture replay callout
...
- Status: [x] DONE
- Zone: GREEN
- Files: `apps/desktop/src/main/overlay-window.ts` (+ test), `apps/desktop/src/preload/overlay.ts` (+ test), `apps/desktop/src/overlay-connection.ts` (+ test), `apps/desktop/src/renderer/{overlay.html,overlay-main.tsx,overlay-global.d.ts}`, `packages/overlay/src/{OverlayApp,VerdictHud,CargoStrip}.tsx` + `{overlay-state,overlay-theme}.ts` + `ws/loopback-client.ts` (+ tests), plus wiring in `main/index.ts`, `main/ws-server.ts` (onConnect), `main/ipc.ts`/`preload/api.ts` (`overlay.toggle`), `renderer/screens/CommandDeck.tsx` (toggle button), `electron.vite.config.ts` (2nd renderer+preload entry), `docs/verification/phase-2.md`.
...
- **IMPLEMENTATION NOTES (built 2026-07-13):** (1) **New `@lodestar/overlay` package** — a self-contained React HUD (`OverlayApp`/`VerdictHud`/`CargoStrip`, inline-styled, no Tailwind) over a pure `overlay-state` fold + the sole sanctioned loopback WebSocket client (`ws/loopback-client.ts`). The `WebSocket` global is re-permitted there in BOTH the eslint egress block AND the compliance raw-socket scanner (`overlay/src/ws`), narrowly — fetch/XHR/raw-sockets stay banned. (2) **No IPC in the overlay:** its preload exposes ONLY the WS connection info (`{port,token}`) parsed from `additionalArguments` (renderer argv, never a URL/query/log); there is no `ipcRenderer`, so telemetry is provably WS-only. (3) **Late-joiner fix:** the WS server gained an `onConnect` hook; main primes each new client with the current `state.snapshot` (its delta baseline) + the latest verdict before any broadcast. Verdicts are now broadcast to WS (not just IPC). (4) **New `overlay.toggle` invoke channel** (closed-union add) drives the Command Deck button; a global shortcut (Ctrl+Shift+O, registration checked) also toggles. (5) **electron-vite** gained a 2nd renderer HTML (`overlay.html`, CSP `connect-src` allows `ws://127.0.0.1:*`) + 2nd preload entry. Coverage: `createOverlayWindow` glue is unit-tested via a mocked BrowserWindow (asserts the click-through `setIgnoreMouseEvents(true)` guarantee).
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/core/src/analytics/{repository.ts,aggregates.ts,index.ts}` + `{repository,aggregates,benchmark}.test.ts`
...
- **IMPLEMENTATION NOTES (built 2026-07-13):** NO migration (3.1 has none; 005 is personal_bests). The hot queries use the EXISTING indexes: all access to the LARGE child tables (`refinements`/`prospects`) goes through `session_id` (idx_refinements_session / idx_prospects_session) — prospect counts as correlated subqueries, the commodity filter as a correlated `EXISTS`, detail breakdowns as `WHERE session_id = ?`; the small `sessions` root is walked in PK order (`ORDER BY s.id DESC` = recency, since ids are assigned in start order) — no full scan of a large table (EXPLAIN-asserted). All derived math (durations, per-hour rates, cross-session fold, filter WHERE) is PURE in `aggregates.ts` (golden-tested without a DB); `repository.ts` is SQL + mapping; `aggregate()`/`trend()` fold the list purely. Filter values are bound params (no injection). History = ENDED sessions only (an active session has no final totals/duration) but `sessionDetail(id)` works for any status. Documented 1k-session benchmark: **list+aggregate+filter+trend in ~10 ms** (functional-only in CI, never timing-gated).

#### Step 3.2 — Per-commodity & per-ring breakdowns
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/core/src/analytics/breakdowns.ts` + tests (+ `breakdowns()` on the analytics repository)
...
- **IMPLEMENTATION NOTES (built 2026-07-13):** PURE folds in `breakdowns.ts` (`foldBreakdown`/`foldPairings`/`assembleBreakdowns`, golden-tested without a DB) + a `breakdowns(filter)` repository method that assembles session inputs from `listSessions` + a per-session dominant-commodity query (index-backed, EXPLAIN-asserted). **Multi-ring "split by time-in-ring" is ALREADY enforced upstream:** the tracker (Step 1.8, `advance`) ends a session and opens a new one when mining resumes at a different ring, so every session is SINGLE-ring and each contributes wholly to its one ring/ship/dominant-commodity — rates are unambiguous. Built: **by commodity (dominant), by ring, by ship + best (ring × commodity) pairings**. **DEFERRED (flagged, data not persisted): "by ring-type" and "by method"** — ring type needs `Scan` data the tracker doesn't log into `session_events`, and method needs the loadout inference deferred past Phase 2; `foldBreakdown` is generic so either drops in as a new key function once the underlying column/signal is stored (candidate for a small follow-up + migration when convenient). Green: core 362 tests + typecheck + lint + compliance.
...
- Status: [x] DONE — **split into 3.5a (data plumbing) + 3.5b (the UI)** because the analytics types had to move to `shared` to cross IPC, and the plumbing is a clean, independently-testable unit. Built LAST in Phase 3 so it surfaces every already-built data feature.
- **DIVERGENCE (flagged): Recharts deferred.** Recharts wasn't installed; rather than pull in a heavy chart dep (with jsdom-test friction) autonomously, the charts are lightweight, dependency-free custom SVG/CSS (`Sparkline`/`TrendChart`/`HeatmapGrid`) — fully DOM-testable and on-brand. The acceptance ("every Phase-3 data feature reachable + tested; zero-data state") is fully met. Richer Recharts charts are a straightforward later polish.
- Zone: GREEN
...
#### Step 3.6 — CSV export
- Status: [x] DONE (built before 3.5 so the Manifest UI can wire the export button)
- Zone: GREEN
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/data/src/migrations/006-galaxy.ts` (inlined SQL const, per the established migration convention — not a standalone `.sql` file), `packages/data/src/repositories/{systems,stations,bodies,rings,hotspots,overlaps,runs,index}.ts` + `repositories/{galaxy,galaxy-intel}.test.ts`
...
- Status: [x] DONE
- Zone: GREEN
- Files: `resources/seed/hotspots-seed.json`, `resources/seed/PROVENANCE.md`, `packages/data/src/seed-import.ts` + `seed-import.test.ts`
- **Note (deferred, flagged):** runtime "load the seed at galaxy init / first run" wiring is intentionally NOT in this step's file list — `parseSeedFile`/`importSeed` are the tested library; the loader is wired by the first consumer (db-service init or the Vein Finder, Step 4.13). Seed coordinates are approximate (`coordsApproximate: true`), authoritatively refined by EDSM (Step 4.7) and the commander's own location — the seed's guaranteed content is ring/hotspot identity + provenance.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/core/src/journal/events/{saa-signals.ts,scan.ts}`, `packages/core/src/hotspots/recorder.ts` + `{saa-signals,scan,recorder}.test.ts`
- **Note (deferred, flagged):** the recorder is independently tested but not yet wired into the live engine — the "engine subscribes Scan/SAASignalsFound → `recorder.record(event, state.location)`" hookup is the integration point (engine wiring / a later step), out of this step's file scope. A scan whose `systemAddress` doesn't match the current location, or that arrives before location is known, is safely **skipped** (never misattributed) — such data is recovered on the next scan or via EDSM enrichment (4.7).
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/intelligence/src/hotspots/overlaps.ts` + `overlaps.test.ts`
- **Note (flagged):** the `overlap_multiplier` values are PROVISIONAL community-documented defaults carrying a provenance note (same convention as `thresholds.ts`) — the exact post-Update-14 yield factors are a Phase-6 Bayesian-calibration target, not verifiable offline; the STRUCTURE (candidate=1.0 no-boost, confirmed monotonic in multiplicity) is final and fully tested. "Confirmed outranks unconfirmed in scoring" is proven at the multiplier level here; the full scoring integration lands in 4.5.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/intelligence/src/scoring/{score.ts,weights.ts}` + `score.test.ts`
- **Note (flagged):** the weight VALUES (`reserveWeights`, `ringAffinity`, `distancePenaltyPerLy`, `sellLegPenaltyPerLs`) are PROVISIONAL community-documented defaults with a provenance note — Phase-6 Bayesian calibration owns updates (bumping `version`). The formula + term breakdown + monotonicity STRUCTURE is final and fully tested.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/integrations/src/gateway/{rate-limiter.ts,cache.ts,backoff.ts,client.ts}` + tests, compliance-suite extension
- **Note (additions, flagged):** `client.ts` (the composed "client-quality layer" the Work sentence describes — cache→rate-limit→gateway→backoff + data-age stamp; what 4.7–4.10 consume) is an addition to the file list. The SQLite cache lives in a self-owned `http_cache` table created via `CREATE TABLE IF NOT EXISTS` — ephemeral, regenerable infra OUTSIDE the forward-only profile-migration chain (the `schema_migrations` precedent), added to §5.5 as such. `@lodestar/data` is a **dev**Dependency of integrations (the `Db` type is a type-only import erased at runtime; the concrete DB is injected). Instantiating the client with a real DB-backed cache store is wired by the 4.7–4.10 API clients / desktop.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/integrations/src/edsm/{parse,client,enrich,fixtures}.ts` + `{parse,client,enrich}.test.ts`
- Work: sphere/cube system search, bodies + rings (type, reserve) enrichment for candidate systems, coordinate backfill — through the gateway, into the galaxy tables + age stamps.
- **Note (clarifications, flagged):** the galaxy tables (`systems`/`bodies`/`rings`) have **no `source` column** (only `hotspots`/`overlaps` do, and EDSM supplies no hotspot data) — EDSM provenance is the `updated_at` age stamp, not a source tag. `enrich.ts` takes the galaxy repositories as **injected params** (type-only `@lodestar/data` import) so the integrations runtime stays native-free. The client isn't yet wired into a persisting service — the Cartographer (4.12) / Vein Finder (4.13) orchestrate fetch→enrich.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/integrations/src/eddn/{commodity-schema,listener,market-sink,fixtures}.ts` + `{commodity-schema,listener,market-sink}.test.ts`
- **Note (deferred transport + additions, flagged):** the listener consumes an **injected async-iterable `EddnSource` port** (tested via a test-double publisher), so NO raw socket appears in this step's code. The concrete **ZeroMQ SUB adapter** to `tcp://eddn.edcd.io:9500` — plus its supply-chain decision (a native `zeromq` dep + `onlyBuiltDependencies` entry) and its egress-firewall sanctioning (eslint socket-ban carve-out + compliance `skipPaths` for `integrations/src/eddn` as the §5.4 sanctioned non-HTTP egress module) — is a **deliberate operator-facing follow-up** (adding a native network dependency overnight is a decision to surface, not slip). `market-sink.ts` (the real `market_snapshots` writer, `source='eddn'`, via type-only injected `Db`) + the source port are additions to the file list. The kill-switch is an injected `isEnabled()`; its settings-key wiring lands with the consumer. Plausibility bands are provisional (Ledger historical band + Phase-6 calibration refine).
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/integrations/src/inara/{parse,client,fixtures}.ts` + `{parse,client}.test.ts`
- Work: inapi/v1 client (API key from secrets; disabled without one), market-reference queries batched per Inara's terms.
- **Note (flagged):** the exact Inara **market-event schema** (`getCommodityPrices` `eventData`) is PROVISIONAL — verify against live Inara before shipping the real query. The API **key is injected** (the consumer reads it from secrets; integrations stays decoupled). POSTs are **uncached** (`ttlMs:0` — auth + per-body); the §5.3 2 h market-reference cache needs body-aware keying (later refinement). `source='inara'` **ingestion into `market_snapshots`** is wired by the Ledger (4.11) which owns market persistence — this step delivers the client + parse. Inara **app registration** (§9) is required for a real key.
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/integrations/src/capi/{oauth.ts,client.ts}` + `{oauth,client}.test.ts`
- **Note (deferred wiring, flagged):** the module is real + tested against a fake of the Frontier endpoints; the **system-browser open + loopback-redirect catcher + `safeStorage` token-store adapter** are APP wiring (the integrations client takes an injected `CapiTokenStore` port). `profile`/`market` return raw JSON — schema parsing is the consumer's job (Ledger/Cartographer). Ships **disabled** until Frontier **client-id approval (§9)**; the flag gates Settings exposure and, when off, blocks any network attempt (tested).
...
- Status: [x] DONE
- Zone: GREEN
- **DECOMPOSITION (2026-07-14, changelog-noted):** this step spans pure intelligence, a core service, the alert framework + migration 007, and a full UI screen, so it is split into **4.11a** (pure `intelligence/ledger` — best-sell ranking + trends), **4.11b** (`core` ledger-service + alert-engine + migration 007 `alert_rules`), and **4.11c** (Ledger UI). Each is atomic, green, and independently reviewed — same pattern as 2.7/3.5.
- **4.11c notes (flagged):** live alert EVALUATION → delivery (price/cargo signals → `engine.evaluate*` → the `emit` → notification + TTS at the `alert` priority) is not yet wired into the live engine — the bridge exposes the `engine` + `emit` (currently logs) for that hookup. `analytics.export`'s `ledger`/`alerts` channels bring the closed Channel union to 27.
- **4.11b notes (flagged):** the alert engine's edge state is in-memory (rebuilt on restart); the persisted `last_fired_ts` + per-rule cooldown mitigate re-fire spam. `ledger-service.bestStations` does not populate `distanceLs` (`market_snapshots` has no player-relative distance) — the distance filter is a no-op until station/system distances are joined (Cartographer 4.12). Alert delivery (notification + TTS at the `alert` priority) is via the injected `emit` callback, wired by the desktop consumer.
...
- Status: [x] DONE
- Zone: GREEN
- **DECOMPOSITION (2026-07-14, changelog-noted):** split into **4.12a** (pure `intelligence/planner` — run-planner + strategies), **4.12b** (Spansh client `integrations/spansh` + `core` plan-service persisting to `runs`), **4.12c** (Cartographer UI + copy-to-clipboard). Same 2.7/3.5/4.11 pattern.
- **4.12c notes (flagged):** `miningTph` is a flat provisional estimate (150) and cargo/jump-range are defaults (per-ship/per-method estimates later); the Cartographer builds candidates by pairing each hotspot with its best Ledger sell (the mining/sell systems must both be in the galaxy DB). The Spansh-backed `RouteProvider` is NOT injected — the bridge uses the pure straight-line provider (the injection point is marked in `index.ts`). Channel union → 29.
- **4.12b notes (flagged):** `core` may not import `integrations`, so `plan-service.ts` defines a `RouteProvider` **port** + a pure `straightLineRouteProvider` fallback; the Spansh-backed provider adapter is INJECTED by the desktop (4.12c/wiring). Spansh endpoint paths + result schema are PROVISIONAL (verify against live). The §5.3 24 h route cache is NOT implemented — Spansh's per-job results URL makes the ApiClient's URL-keyed cache useless; the ≤6 req/min rate limit + ≥2 s poll interval are enforced (later: a query-keyed route cache).
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/shared/src/vein.ts`, `packages/core/src/veins/vein-service.ts` (the scored-candidate builder — a flagged addition, since desktop can't import `intelligence`), `apps/desktop/src/main/vein-wiring.ts`, `apps/desktop/src/renderer/{screens/VeinFinder.tsx,nav.ts}` + tests
- **Note (additions/flagged):** the candidate builder lives in `core` (`createVeinService` — `core → intelligence` is allowed; desktop wires location + filter). The **"Plan this" handoff** navigates to the Cartographer via a tiny `nav.ts` registered by `App`, passing the commodity; it does NOT auto-select the exact run in the planner (a lightweight pre-fill — deeper pre-fill is a refinement). The `maxSellLegLs` filter is a no-op until per-station distances are stored. Channel union → 30.
...
- Status: [ ] TODO — **CI-testable core DONE** (geometry + 2D schematic fallback + selection + WebGL detection); the **react-three-fiber 3D `Canvas` + the 200-marker fps benchmark are a flagged operator follow-up** (below)
- Zone: GREEN
- Files: `apps/desktop/src/renderer/components/ringmap/{geometry.ts,webgl.ts,RingSchematic.tsx,RingMap.tsx}` + tests
- **Note (deferred 3D, flagged):** the 3D scene requires adding a large rendering dependency (`three` + `@react-three/fiber` + `@react-three/drei` for orbit controls) and its render CANNOT run in CI/jsdom, and the fps target is a §4.2 MANUAL verification — so introducing that dep + the fps check overnight is a deliberate **operator-facing decision**, not slipped in. Built + fully tested instead: the pure `geometry.ts` marker-placement math (shared by both renderers), the **real 2D SVG `RingSchematic`** (annulus + count-sized/commodity-coloured markers + overlap rings + click/keyboard selection + legend + empty state — the SSOT's "real implemented fallback"), `webgl.ts` capability detection, and the `RingMap` container that chooses the renderer (2D today; the 3D `Canvas` drops into the pending branch using the SAME `layoutMarkers` geometry, so selection/placement behaviour won't change). Never touches CUDA. Wiring `RingMap` into the Vein Finder (its `selected`/`onSelect` sync props are ready) is a small follow-up. Acceptance met: scene-graph math + selection headlessly tested; no-WebGL → the labelled 2D schematic. Outstanding: the 3D `Canvas` render + the manual fps benchmark (`docs/verification/phase-4.md`).
...
- Status: [ ] TODO (4.15a DONE)
- Zone: GREEN
- Files: `packages/intelligence/src/outfitter/{templates.ts,advisor.ts}`, `apps/desktop/src/renderer/screens/Outfitter.tsx` + tests
- **DECOMPOSITION (2026-07-14, changelog-noted):** split into **4.15a** (pure `intelligence/outfitter` — templates + advisor) and **4.15b** (Outfitter UI screen). Same pattern.
- **4.15a note (flagged):** module internal-name substrings + per-method required/recommended sets are community-documented (verified in-game). The `ShipSlots` fit-check is validated when slot data is passed; sourcing the ship's full slot table (from the loadout's `Ship` + a ship-slot DB) is the desktop's job (4.15b/wiring) — until then suggestions aren't slot-filtered (documented).
...
- 2026-07-14 — **Step 4.15a done — pure Outfitter (templates + advisor).** `packages/intelligence/src/outfitter/`: `templates.ts` (per-method reference module sets — `laser` / `deep-core` / `subsurface` — each module a `ModuleSpec` {kind, label, hardpoint|optional-internal, minSize, and the journal `Item` substring that identifies it}; `METHOD_TEMPLATES` required vs recommended; `detectModule(item)` matches an equipped module) and `advisor.ts` (`analyzeLoadout(modules, method, shipSlots?)` → what's equipped, the **missing REQUIRED** modules with reason "required for <method>" — e.g. no PWA for deep-core — and **recommended** additions; suggestions are validated against `ShipSlots` so a recommendation **never names a module that doesn't fit**, while required gaps are always surfaced with a `fitsShip` flag so an impossible ship is visible not hidden). 87 intelligence tests: module detection, gap analysis for all three methods (empty rig misses every required; a full deep-core rig has none; a laser rig lacks PWA+seismic for deep-core), and the slot-fit rules (drop an unfittable recommendation, flag an unfittable requirement, no filtering when slots unknown). **DECOMPOSITION** of 4.15 (→ 4.15a pure / 4.15b UI). **Flagged:** internal-name substrings + method sets are community-documented; the ship-slot table sourcing is 4.15b/wiring. lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.14 core done — ring/hotspot map (geometry + 2D schematic + WebGL fallback); 3D `Canvas` flagged.** `apps/desktop/src/renderer/components/ringmap/`: `geometry.ts` (pure `layoutMarkers` — hotspots placed evenly around the annulus starting at −90°, sized by clamped signal count, coloured by a deterministic per-commodity hue; shared by both renderers, headlessly tested), `webgl.ts` (`hasWebGL` capability probe, injectable, never throws), `RingSchematic.tsx` (the real 2D SVG ring view — annulus + markers + overlap highlight rings + click/keyboard **selection** + colour legend + empty state), `RingMap.tsx` (container choosing the renderer by GPU: 2D today, a `3d-pending` branch reserved for the r3f `Canvas`). 15 ring-map tests (marker placement math, size clamping, deterministic colour, selection via click + keyboard, aria-pressed, empty ring, WebGL true/false/throws, fallback rendering). 311 desktop tests. **Flagged/deferred (operator decision):** the react-three-fiber 3D `Canvas` needs the `three`+r3f dependency and a non-CI manual fps benchmark (§4.2) — surfaced, not slipped in; the 2D schematic is the working default and the acceptance's real fallback. Never touches CUDA. lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.13 done — Vein Finder UI + filters.** `packages/shared/src/vein.ts` DTOs (score breakdown mirrors 4.5 term-for-term) + the `veins.find` channel (union → 30). `packages/core/src/veins/vein-service.ts` (`createVeinService`: joins each hotspot → ring type/reserve + system coords + overlap state (4.4) + best Ledger sell (4.11), scores it with the Step-4.5 `scoreRing`, exposes the full breakdown, measures distance from the origin (null if unknown); candidate overlaps surface as "possible" and give NO boost, only confirmed boost). Main `vein-wiring.ts` (thin — origin from the live location) + ipc arg-validated `veins.find` + preload. Renderer `screens/VeinFinder.tsx` — a **ranked** hotspot list with the full **"why" breakdown**, composable **filters** (commodity/max-distance/reserve/ring-type/pad), honest **overlap badges** (confirmed ✓ vs "possible — verify in ring"), data-age + provenance source, an explicit **seed-only** first-run banner, and a **"Plan this" → Cartographer handoff** (via a tiny `nav.ts` registered by `App`); `vein-finder` module flipped available + routed. 296 desktop + 6 core vein tests (service: score/rank + breakdown-mirrors-4.5 + overlap state + confirmed-boost + filter composition + null-distance + provenance; screen: render/filters-compose/handoff/seed-only/empty/error/confirmed-badge; bridge + ipc). **Flagged:** candidate builder added in `core`; handoff doesn't auto-select the exact run; sell-leg filter a no-op pending station distances. lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.12c done — Cartographer UI. STEP 4.12 COMPLETE.** `packages/shared/src/planner.ts` DTOs + 2 channels (`planner.plan`/`planner.save` — payloads + set + exact-list channels.test in sync, union → 29). Main: `cartographer-wiring.ts` (`createCartographerBridge`: builds round-trip PLAN REQUESTS by pairing each galaxy hotspot with its best Ledger sell station — routing to it via the core plan service's straight-line provider — ranks by strategy, holds the last list so `save(index)` persists to `runs`) + `ipc.ts` arg-validated `planner.plan`/`planner.save` + `index.ts` wiring (the Spansh-provider injection point marked). Preload gains the 2 methods. Renderer: `screens/Cartographer.tsx` — a **Max Profit / Min Time / Safest** strategy picker (refetches), ranked plan cards (cr-hr/t-hr/round-trip/jumps + the leg-by-leg breakdown + mine→sell line), **Copy route → PLAIN SYSTEM NAMES** (`planWaypoints`, verified `writeText("Paesia\\nSol\\nPaesia")` — **no game injection**), Save plan, and zero-data/error states; `cartographer` module flipped available + routed. 285 desktop tests (screen: cards/strategy-refetch/copy-plain-names/save/empty/error/copy-fail/save-null; bridge: hotspot↔sell candidate build + rank + persist + skip-no-price + empty; ipc: planner handlers + arg-rejection). **Flagged:** flat provisional miningTph + default ship params; Spansh provider not yet injected (straight-line). lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.12b done — Spansh client + plan service.** `packages/integrations/src/spansh/client.ts` (`createSpanshClient`: the Spansh **async job** flow — submit a `/api/route` job → poll `/api/results/<id>` at ≥ 2 s intervals (§5.3) until done → parse jumps + distance, all through the Step-4.6 ApiClient so ≤ 6 req/min + backoff apply; tolerant parsing; `spansh/timeout` after `maxPolls`). `packages/core/src/planner/plan-service.ts`: `createPlanService(db, routeProvider)` builds `RunCandidate`s (legs from an **INJECTED `RouteProvider`** — `core` can't import `integrations`), ranks via the 4.12a `rankPlans`, and `savePlan` persists to `runs` (status `planned` + estimates); ships a pure `straightLineRouteProvider` (Euclidean distance, `jumps = ceil(distance/range)`, default security) as the real no-network fallback. **Still no game control** — routes are data. 17 new tests (integrations 178, core 447): async submit→poll→done + timeout + missing-job, straight-line math, ranked planning through the provider, route-failure propagation, and runs persistence. **Flagged:** Spansh schema provisional; 24 h route cache deferred; Spansh→provider desktop adapter is 4.12c. lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.12a done — pure run planner (round-trip).** `packages/intelligence/src/planner/`: `run-planner.ts` (`planRun(candidate, input)` computes the full **mine here → sell there → return** estimate — fill time = cargo/tph, travel = totalJumps × secondsPerJump, cargo value, effective tph/cph over the whole trip, all divide-by-zero-guarded; `rankPlans(candidates, input, strategy)`) and `strategies.ts` (the three orderings — **Max Profit** cr/hr, **Min Time** duration, **Safest** highest min-route-security then fewest jumps). **Pure + no game control** — a plan is data the player copies by hand (Spansh legs are passed IN by the caller; the module never routes/injects). 70 intelligence tests: golden estimate math, the three strategy orderings on a fixture galaxy each picking a distinct winner, the safest jumps-tiebreak, and divide-by-zero guards. **DECOMPOSITION** of 4.12 (→ 4.12a pure / 4.12b Spansh+plan-service / 4.12c UI). lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.11c done — Ledger UI. STEP 4.11 COMPLETE.** `packages/shared/src/ledger.ts` (the IPC DTOs) + 7 new closed-union channels (`ledger.board`/`stations`/`trend`, `alerts.list`/`add`/`setEnabled`/`delete` — ChannelPayloads + CHANNEL_SET + the exact-list channels.test all in sync). Main: `apps/desktop/src/main/ledger-wiring.ts` (`createLedgerBridge`/`emptyLedgerBridge` — the core ledger-service + alert-engine → DTO adapter, board `best undefined→null`, alert CRUD returning the updated list) + `ipc.ts` handlers with arg validation + `index.ts` wiring (alert `emit` currently logs). Preload `api.ts` + `EXPOSED_API_KEYS` gain the 7 methods. Renderer: `screens/Ledger.tsx` + `components/ledger/panels.tsx` — the commodity **board** (best price + data-age badge per commodity), the **station ranking** with **SOURCE + DATA-AGE on every row** (the acceptance), a price **trend** SVG, and the **alert manager** (price-threshold + cargo-full add / enable-toggle / delete); zero-data + error + loading states; `ledger` module flipped available + routed. 270 desktop tests (screen: board→drill, source+age, cargo/price alert CRUD, empty/error states; ipc: 7 handlers + arg-rejection; bridge: DB-fed board/filters/trend + CRUD) + channels exact-list. **Flagged:** live alert evaluation→delivery wiring deferred. lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.11b done — Ledger service + alert framework + migration 007.** Migration 007 (`alert_rules`: `price-threshold` + `cargo-full` kinds with CHECK constraints, direction, per-rule cooldown, enabled, `last_fired_ts`; applies cleanly over Phase 2–6 data — tested). `core/alerts/alert-engine.ts` (`createAlertEngine(db, emit)`: rule CRUD + **edge-triggered firing** — once per crossing, re-arms on leaving the zone — AND **cooldown throttle** (persisted `last_fired_ts`); `evaluatePrice`/`evaluateCargo`; each fire → the injected `emit` = the notification+TTS `alert`-priority delivery point). `core/market/ledger-service.ts` (`createLedgerService(db, now)`: DB→intelligence adapter — `bestStations`/`trend`/`board` over `market_snapshots` feeding the 4.11a ranking, so a fresh journal price beats a stale-but-higher EDDN one end-to-end). 13 new tests (data 59, core 441): 007 over populated data + CHECK, edge/cooldown/cargo/below-direction/disabled/emit, and DB-fed ranking + pad filter + board + trend. **Flagged:** edge state is in-memory (mitigated by persisted last_fired + cooldown); `distanceLs` filter is a no-op until station distances are joined (4.12); delivery wired by the desktop consumer. lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.11a done — pure Ledger intelligence (best-sell + trends).** `packages/intelligence/src/ledger/`: `best-sell.ts` (`rankSellStations(snapshots, now, filter?, weights?)` — pure, demand/pad/distance-filtered, scored `sellPrice × sourceTrust × freshnessWeight`: a first-party `journal`/`capi` price outranks a same-value `eddn` price (spoofable firehose = advisory), and a fresh price beats a stale-but-higher one; versioned `DEFAULT_SELL_WEIGHTS` + `freshnessWeight` linear decay) and `trends.ts` (`priceTrend(snapshots, bucketMs)` time-bucketed avg/max/samples). 63 intelligence tests incl. the two required golden orderings (freshness-beats-stale, journal-beats-EDDN), pad/distance/demand filters, unknown-source trust, and trend bucketing. **DECOMPOSITION** of 4.11 (→ 4.11a pure / 4.11b core+migration007 / 4.11c UI), same 2.7/3.5 pattern. lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.10 done — Frontier cAPI (OAuth PKCE, feature-flagged). All Phase-4 external clients (4.6–4.10) complete.** `packages/integrations/src/capi/`: `oauth.ts` (pure PKCE via `node:crypto` — `generatePkce` derives an S256 challenge from the verifier + anti-CSRF state with injectable randomness; `buildAuthorizeUrl` targets `auth.frontierstore.net` with S256 PKCE; `exchangeCodeForToken`/`refreshAccessToken` POST the token endpoint through the Step-4.6 ApiClient and compute an absolute expiry; `isTokenExpired` with a refresh skew); `client.ts` (`createCapiClient`: `profile`/`market` from `companion.orerve.net` with a Bearer token, **refreshing an expired token via the refresh token and persisting it ONLY through the injected `CapiTokenStore`**; `ttlMs:0` live-no-cache per §5.3; **flag-off → zero network attempts**). 173 integrations tests: PKCE S256 derivation vs an independent hash, authorize-URL params, code/refresh POST bodies, token-expiry + skew, flag-off/no-token making no network call, Bearer-authed profile/market, expired-token refresh persisting via the store, and refresh-failure propagation. **Flagged:** browser-open + loopback catcher + safeStorage adapter are app wiring; profile/market schema parsing is the consumer's; ships disabled until Frontier client-id approval (§9). lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.9 done — Inara client (feature-flagged).** `packages/integrations/src/inara/`: `parse.ts` (pure — validates the inapi/v1 envelope, a header status ≥400 like an invalid key → typed `inara/api-error`; maps market `eventData` to canonical commodity prices, dropping non-mining + malformed rows); `client.ts` (`createInaraClient({api, config, nowIso})`: **disabled without an API key** — `isEnabled()` false and `commodityPrices()` returns `inara/disabled` making NO network call; batches queries into as few inapi/v1 POSTs as the per-request cap allows, all through the Step-4.6 ApiClient so the §5.3 ≤2 req/min limit + backoff apply; API key travels only in the POST body); `fixtures.ts` (recorded envelope). 158 integrations tests: disabled-without-key (no fetch), enabled-with-key, POST envelope carries the key, batching splits by cap, recorded-fixture parse, API-error + bad-JSON surfaced, and the ≤2/min rate policy enforced end-to-end. **Flagged:** the Inara market-event schema is provisional; `source='inara'` market_snapshots ingestion wired by the Ledger (4.11); app registration (§9) needed for a real key. lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.8 done — EDDN listener.** `packages/integrations/src/eddn/`: `commodity-schema.ts` (pure — validates the `commodity/3` envelope + shape, keeps only canonical mining commodities (Step 2.2) passing PLAUSIBILITY: finite/positive/within a provisional per-commodity price band + sane demand; drops unknown/implausible/spoofed rows with a count — the "unauthenticated spoofable firehose" hardening); `listener.ts` (real `node:zlib` frame inflate; pipeline decode→schema→plausibility→sink; **reconnect with jittered backoff**, **kill-switch** (`isEnabled`), **rate-bounded ingestion** (drop-over-window), telemetry on every drop — all driven over an **injected async-iterable `EddnSource` port**, so no raw socket is in this code); `market-sink.ts` (real `market_snapshots` writer, `source='eddn'` upsert on the §5.5 unique key + demand, `prune(cutoff)` retention — via a type-only injected `Db`); `fixtures.ts` (recorded-shape frames). 144 integrations tests: real zlib round-trip of recorded frames→ingest, malformed/wrong-schema/implausible (10×/negative/NaN/absurd-demand) all rejected, reconnect + kill-switch + rate-window + stream-error against a test-double publisher, and market_snapshots upsert/prune against a real in-memory DB. **Flagged:** the concrete ZeroMQ transport adapter + its native `zeromq` dep / `onlyBuiltDependencies` / eslint+compliance socket-sanctioning is a deliberate operator follow-up; kill-switch settings-key + plausibility-band calibration wired later. lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.7 done — EDSM client.** `packages/integrations/src/edsm/`: `parse.ts` (pure — validates EDSM `sphere-systems` (name+coords, coordless entries skipped) and `bodies` (ring type + reserve; normalizes EDSM's `Metal Rich`→`MetalRich`; drops malformed rings), typed errors never throw); `client.ts` (`createEdsmClient(apiClient)`: `sphereSystems`/`systemBodies` through the Step-4.6 ApiClient so 429/cache/rate-limit are handled — coords TTL 30 d, bodies 7 d per §5.3; radius clamped to EDSM's 100 ly; parses the response body); `enrich.ts` (folds parsed data into the galaxy tables via INJECTED repositories — `enrichSystemsFromEdsm` backfills coordinates, `enrichBodiesFromEdsm` fills ring type + reserve onto an existing system, skipping when the system is unknown; type-only `@lodestar/data` import keeps the integrations runtime native-free); `fixtures.ts` (hand-authored recorded-shape EDSM payloads). 115 integrations tests (parse over recorded fixtures, 429-retry + bad-JSON + refusal propagation against a fake server through the real gateway, enrichment into a real in-memory DB). **Flagged:** galaxy tables have no `source` column (EDSM provenance = `updated_at`); fetch→enrich orchestration wired later (4.12/4.13). lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.6 done — Gateway client features (cache, rate limits, backoff).** The client-quality layer on the Step-0.10 enforcement core, in `packages/integrations/src/gateway/`: `rate-limiter.ts` (per-host token buckets, deterministic injected clock, §5.3 sustained limits encoded — EDSM 10/min, Spansh 6/min, Inara 2/min, cAPI 1/5min, fallback 4/min); `backoff.ts` (full-jitter exponential backoff, retries only 429/5xx, honours `Retry-After`, injected sleep/rand/now); `cache.ts` (per-source-TTL response cache with ETag→`If-None-Match` / Last-Modified→`If-Modified-Since` revalidation; a pure freshness/conditional/age API over a `CacheStore` interface with an in-memory store AND a SQLite store owning its own `http_cache` table); `client.ts` (the composition — cache→rate-limit→gateway→backoff with a **data-age stamp on every payload**: fresh hit skips the network, stale hit revalidates (304→serve cached + refresh TTL), rate-limited serves stale-or-refuses, only 2xx cached, and a gateway allowlist/redirect refusal is propagated WITHOUT retry). 89 integrations tests (each feature against a fake server through the real gateway). **Compliance extended:** the FULL composed client is asserted to still refuse every denied-AI + unknown host without a transport hit (25 compliance tests). **Flagged:** `client.ts` + the `http_cache` infra table are additions (recorded in §5.5 + step note); `@lodestar/data` is a type-only devDep of integrations; real-DB cache wiring lands with the 4.7–4.10 clients. lint + typecheck + compliance + full test suite green.
- 2026-07-14 — **Step 4.5 done — Hotspot scoring function (pure, TDD).** `packages/intelligence/src/scoring/score.ts`: `scoreRing(input, weights?)` computes `score = price × overlap_multiplier × reserve_weight × ring_match − distance_penalty − sell_leg_penalty` and returns a `ScoreBreakdown` exposing EVERY term for the "why this score" UI — `base` (the multiplicative product) and `score` (base − the two penalties) reconcile EXACTLY (tested with `toBe`). Confirmed overlaps boost via 4.4 `overlapMultiplier`; candidates provably don't (proven end-to-end at the score level — the 4.4 acceptance). Neutral defaults for unknown reserve/ring-type/commodity (never NaN); negative price/distances clamped to 0. `weights.ts`: versioned `DEFAULT_SCORING_WEIGHTS` (reserve weights Pristine→Depleted, per-commodity ring-type affinity, distance/sell-leg penalties) with a provenance note. 53 tests, 100% branch: exact breakdown reconciliation, monotonicity properties (↑price⇒≥, ↑distance⇒≤, Pristine≥…≥Depleted, ring-match hit>miss, confirmed>candidate=none), and a 4-candidate golden ranking matching hand-order. **Flagged:** weight VALUES are provisional Phase-6 calibration targets; the formula/structure is final. lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.4 done — Overlap model + candidate detection (pure).** `packages/intelligence/src/hotspots/overlaps.ts`: honest overlap modelling — journals give hotspot counts, not positions, so overlap is epistemically invisible from scans. `detectOverlapCandidate(hotspots)` flags a **candidate** iff ≥2 DISTINCT commodities share a ring (deduped, sorted, `multiplicity = commodities.length`), unranked — no likelihood claim. `confirmOverlap(overlap, source?)` promotes candidate→confirmed (idempotent). `overlapMultiplier(overlap, weights?)`: a **candidate always returns exactly 1.0** (early return before the weights table — provably no score boost), only a **confirmed** overlap stacks yields via `DEFAULT_OVERLAP_WEIGHTS.byMultiplicity` (versioned, monotonic non-decreasing, carrying a provenance note; clamped, and degrades to 1.0 on an empty table — never NaN). 42 tests, overlaps.ts 100% line/branch: candidate no-boost, confirm transitions, monotonicity, confirmed-outranks-candidate, empty-weights guard. **Flagged:** multiplier VALUES are provisional Phase-6 calibration targets; the honesty STRUCTURE is final. Full scoring integration in 4.5. lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.3 done — Hotspot recorder from own scans.** Two PURE interpreters in `core/journal/events`: `saa-signals.ts` (`ringBodyName` parses the parent body out of a `"… <letter> Ring"` name; `commodityFromSaaSignal` maps `$SAA_SignalType_…;` → canonical id via the Step-2.2 dictionary, returning undefined for non-mineral signals; `interpretSaaSignals` yields ring+body+mineral hotspots or undefined for a surface/no-mineral event) and `scan.ts` (`normalizeRingClass` folds `eRingClass_…`, tolerating the game's `Metalic` misspelling, → `Icy|Rocky|Metallic|MetalRich`; `normalizeReserveLevel` `…Resources` → `Pristine|…|Depleted`; `interpretRingScan` drops stellar belts, keeping true rings with type+reserve). `core/hotspots/recorder.ts` (`createHotspotRecorder(db)`) folds the commander's OWN `Scan`/`SAASignalsFound` into the galaxy tables (`source='journal'`) in one transaction, linking to a system via the CURRENT location (name+coords) **only when the event's `systemAddress` matches** — else a typed skip (`no-location|system-mismatch|no-rings|no-minerals`), never misattributed; re-scan upserts (repo COALESCE) refreshing `count`+`last_confirmed`, keeping `first_seen`, no duplicate rows. A Scan-then-SAA sequence lands both scoring inputs (type+reserve) and hotspots on one ring row (tested). 41 new tests (429 core); recorder 100% line/95% branch, interpreters 100%. **Flagged:** live-engine wiring deferred to the integration point. lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.2 done — Hotspot seed dataset + import (clean-room).** `resources/seed/hotspots-seed.json`: a deliberately-minimal, hand-authored starter set of iconic, community-common-knowledge mining systems (Painite/LTD sites) — **no data copied from any tool**; `resources/seed/PROVENANCE.md` states the clean-room policy + sources. `packages/data/src/seed-import.ts`: `parseSeedFile(unknown) → Result<SeedFile, DomainError>` deep-validates the shape and every commodity id against the canonical dictionary (Step 2.2) with per-level typed error codes (`seed/not-object|bad-version|no-systems|bad-system|bad-coords|bad-body|bad-ring|missing-provenance|bad-hotspot|unknown-commodity`); a ring with blank provenance is rejected. `importSeed(db, seed, at) → summary` folds system→body→ring→hotspot via the 4.1 repositories in one transaction, `source='seed'`, **idempotent** (re-import upserts — no duplicate rows — keeping `first_seen`, refreshing `last_confirmed`; tested). A test reads the SHIPPED json and asserts every ring carries provenance, every system is honestly flagged `coordsApproximate:true`, every commodity resolves, and a double-import is a no-op. 58 data tests; seed-import.ts 100% line / 99% branch. **Flagged:** seed coordinates are approximate public reference, EDSM-refined (Step 4.7); runtime seed-loading wiring deferred to the first consumer (db-service init / Vein Finder 4.13) — out of this step's file scope. lint + typecheck + compliance green.
- 2026-07-14 — **Step 4.1 done — Galaxy reference schema + repositories. PHASE 4 (Prospector Intelligence) begins.** Migration 006 (`006-galaxy.ts`, inlined SQL const per convention) creates seven galaxy tables — `systems` (address UNIQUE, stored `x/y/z` coords), `bodies`, `rings` (ring type + reserve), `stations` (pad size, market id, distance), `hotspots` (count + provenance `seed|journal|community`, UNIQUE(ring,commodity)), `overlaps` (JSON commodity array, `candidate|confirmed` confidence), `runs` (planner estimates vs actuals) — and **additively** extends `market_snapshots` (`ADD COLUMN pad_size, demand` + `idx_market_system`) with **no rebuild** of the table already holding Phase 2–3 user data (tested: v6 applies cleanly over a populated DB and the pre-existing row survives untouched with NULL new columns). Seven typed repositories (+ barrel) with COALESCE-merge upserts (`ON CONFLICT … DO UPDATE`, RETURNING id) and the hot queries index-backed: `systems.within(center, radiusLy)` cube-prefilters on `idx_systems_x` then Euclidean-refines + sorts nearest-first; ring-by-system joins via `idx_bodies_system`; hotspot-by-commodity seeks `idx_hotspots_commodity` — each `EXPLAIN QUERY PLAN` asserted in test to use its index (no full scan). A corrupt `overlaps.commodities` blob degrades to `[]` (a read never throws). Documented benchmark (not a gate): `within(50 ly)` over 20k seeded systems ≈ 4.3 ms. 31 data tests; branches 98.03% / functions 100%; full lint + typecheck + compliance green. Scoring-facing columns (overlap multiplicity, reserve, ring type) are stored now and consumed by 4.4/4.5.
- 2026-07-14 — **Step 3.5b done — Manifest dashboards UI. PHASE 3 (Manifest/Analytics) FEATURE-COMPLETE.** `apps/desktop/src/renderer/screens/Manifest.tsx` + `components/manifest/*` on the deck design language (ScreenHeader + glass): KPI totals, personal-best board, session history table (tons/hr sparkline + click-to-drill-down), tons/hr trend line, day×hour + ring×commodity heatmaps, per-ring/commodity/ship breakdowns + best pairings, limpet + time-split efficiency, and CSV export buttons (3.6). Fetches the bundle over IPC (`getManifest`) on mount; **explicit zero-session first-run state** + loading + error states designed. `manifest` module flipped available + registered in routes (nav-drift test repointed to Vein Finder as the unbuilt example). **Recharts DEFERRED (flagged):** not installed — the charts are lightweight, fully-testable custom SVG/CSS instead (Sparkline/TrendChart/HeatmapGrid), meeting the "every data feature reachable + tested + zero-data" acceptance with no bundle weight; richer Recharts is a later polish. ~30 component tests (full-bundle render asserting every feature, drill-down, CSV export click, zero-data, error, + all empty/edge branches). Desktop 249 tests; full test + typecheck + lint + compliance + e2e (6/6) green.
- 2026-07-14 — **Step 3.5a done — Manifest data plumbing (types→shared + IPC).** The analytics DATA types (SessionListItem/Aggregates/Breakdowns/Heatmaps/PersonalBest/SessionEfficiency/TrendPoint/SessionDetail/SessionFilter + the `ManifestData` bundle) now live in `packages/shared/src/analytics.ts` — they cross the IPC boundary, and `shared` is the only package both `core` (producer) and the renderer (consumer) may depend on. **Low-risk approach:** the merged `core/analytics` files are UNTOUCHED (their local types are structurally identical); the main-process `analytics-manifest.ts` `buildManifest` returns the shared `ManifestData`, which COMPILE-ENFORCES the core→shared shape match at that one boundary (drift = a type error). New pure `core` helper `foldPersonalBests(inputs)` folds the whole ended-session history into the current records for the board (the persisted store still drives the live `session.newBest`; single-rock value stays 0 until per-prospect value is persisted — flagged). New invoke channels `analytics.manifest` (SessionFilter → bundle) + `analytics.sessionDetail` (id → detail|null) + preload `getManifest`/`getSessionDetail`; wired in `index.ts` (empty bundle when no DB). Full test + typecheck + lint + compliance green.
- 2026-07-14 — **Step 3.7 done — limpet efficiency + time split. PHASE 3 data layer complete (only the 3.5 UI remains).** `packages/core/src/analytics/limpets.ts` (PURE): `sessionLimpets(events, tons)` counts LaunchDrone by drone type from `session_events` payloads → prospector/collection/other launches + `collectorProductivity = tons ÷ collection limpets` (0 if none). **Definition of record honoured:** journals emit NO per-fragment collection events, so ONLY launched counts + the productivity ratio are reported — never a fabricated "collected" count. **Reconciliation tested:** prospector+collection+other launches == the session's stored `limpets_launched`. `time-split.ts` (PURE): `sessionTimeSplit(events, started, ended, gapThresholdSec=300)` segments duration into mining vs other from mining-event cadence (consecutive mining events ≤ threshold apart = active mining; larger gaps = travel/sell) — the Phase-1 classifier's per-moment flags aren't persisted, so this is a documented cadence heuristic; `miningSec + otherSec == durationSec` (reconciled). Repository `sessionEfficiency(filter)` fetches each session's events (index-backed `SESSION_EVENTS_SQL`, EXPLAIN-asserted) + folds both, with pooled totals. Golden fixtures pin the exact per-session values. Core 387 tests; full test + typecheck + lint + compliance green.
- 2026-07-14 — **Step 3.6 done — CSV export (RFC-4180 + save dialog).** `packages/core/src/analytics/csv-export.ts` (PURE): `toCsv`/`parseCsv` (RFC-4180 — CRLF records, fields with comma/quote/CR/LF quoted, embedded quotes doubled, null → empty, optional UTF-8 BOM) + typed `sessionsCsv`/`refinementsCsv`/`prospectsCsv`. **Round-trip proven** (tricky values incl. embedded comma/quote/newline re-parse identically) + BOM. Desktop `main/analytics-export.ts` `createAnalyticsExporter({db, showSaveDialog, writeFile})` — builds the dataset, shows the native save dialog, writes; dialog+fs INJECTED so it unit-tests without Electron/disk (cancel path + BOM covered). New `analytics.export` invoke channel (validated kind) + preload `exportAnalytics`; `index.ts` wires the real electron `dialog.showSaveDialog` + `fs/promises writeFile` (guarded on db-ok). **Built before 3.5** so the Manifest UI can add the export button. Core + data + desktop suites + typecheck + lint + compliance green.
- 2026-07-13 — **Step 3.4 done — personal bests (migration 005).** `packages/data/src/migrations/005-personal-bests.ts` (`personal_bests`, category = PK, value + context ship/ring/date/session_id, §5.5). `packages/core/src/analytics/personal-bests.ts` `createPersonalBestsStore(db)`: `check(input)` evaluates an ended session against the records in ONE transaction, replaces only STRICTLY-beaten ones (equalling a record does not replace it), and returns exactly the newly-beaten records — that list IS the `session.newBest` payload (each new best once; re-checking the same session beats nothing — tested). Categories: tons/hr, credits/hr, single-rock value, longest session, most tons. A 0/NaN value never records a best. `sessionBestValues` builds the value set from a session's totals + the best single-rock value. **Wiring note (flagged):** the actual EMISSION of the `session.newBest` channel + the single-rock-value computation (session prospects × the live price book) land at the engine/UI integration (3.5) — the core store + comparison are complete and tested here. Boundary + emit-once + FK (session must exist) tested. Core 373 + data 14 tests; full test + typecheck + lint + compliance green.
- 2026-07-13 — **Step 3.3 done — productivity heatmaps (data).** `packages/core/src/analytics/heatmaps.ts` (PURE): `timeProductivityHeatmap` (day-of-week × hour-of-day tons/hr, attributed by each session's START time, bucketed in UTC — local presentation is a UI offset) + `ringCommodityHeatmap` (ring × commodity refined-tons yield) → a `{rows, cols, cells}` matrix. **Empty-cell semantics explicit:** a cell is `null` for "no session/pairing" vs a real numeric value (0 distinguished from absent). `heatmaps(filter)` repository method feeds them from `listSessions` + a ring×commodity yield query (`HEATMAP_YIELD_SQL`, joins refinements via idx_refinements_session — EXPLAIN-asserted). Golden fixture pins two same-weekday/hour sessions collapsing into one 35 t/h slot + the ring×commodity matrix (Hyades-painite = null). Unparseable timestamps are skipped, never thrown. Core 367 tests; full test + typecheck + lint + compliance green.
- 2026-07-13 — **Step 3.2 done — per-commodity/ring/ship breakdowns + best pairings.** `packages/core/src/analytics/breakdowns.ts` (PURE: `foldBreakdown`/`foldPairings`/`assembleBreakdowns`, golden-tested no-DB) + `breakdowns(filter)` on the analytics repository (assembles session inputs from `listSessions` + a per-session dominant-commodity query, index-backed/EXPLAIN-asserted). **Key data-model insight:** the tracker (1.8) already splits a session on ring change, so every session is SINGLE-ring — "split by time-in-ring" is enforced upstream and each session contributes wholly to its one ring/ship/dominant commodity (rates unambiguous). Views: by commodity (dominant), by ring, by ship, + top (ring × commodity) pairings ranked by tons/hr. **DEFERRED + flagged for the operator:** "by ring-type" and "by method" — NOT persisted (ring type needs `Scan` data not logged into `session_events`; method needs loadout inference deferred past Phase 2); the generic fold accepts either as a new key fn once that data is stored (a small follow-up + migration when convenient). Self-review: no BLOCKING (single-ring invariant verified in the tracker; dominant-commodity tiebreak deterministic; rates never divide by zero). Core 362 tests; full test + typecheck + lint + compliance green.
- 2026-07-13 — **Step 3.1 done — historical query layer. PHASE 3 (Manifest/Analytics) opens.** `packages/core/src/analytics/`: `createAnalyticsRepository(db)` — read-only history over the user's OWN sessions/refinements/prospects. `listSessions(filter)` (ended sessions, newest-first, with derived duration + tons/hr + credits/hr + prospect/MINE counts; filter by date/system/ring/commodity), `sessionDetail(id)` (any status — per-commodity refined tons + prospect summary: prospected/MINE/acted-on/motherlodes), `aggregate(filter)` (cross-session totals + rate averages + hit rate), `trend(filter)` (chronological productivity series). **Split:** `aggregates.ts` is PURE (durations, `perHour`, `computeAggregates`, `buildSessionWhere`, row mappers) — golden-tested with no DB; `repository.ts` is SQL + mapping. **Index discipline (acceptance):** NO migration (005 is personal_bests); every LARGE-table (`refinements`/`prospects`) access goes through `session_id` (idx_refinements_session / idx_prospects_session — prospect counts as correlated subqueries, commodity filter as a correlated `EXISTS`, detail as `WHERE session_id = ?`); the small `sessions` root walks PK order (`ORDER BY id DESC` = recency); `EXPLAIN QUERY PLAN` asserted in CI to use those indexes (no large-table scan). Golden fixture (2 ended + 1 active seed) pins every aggregate/filter/detail/trend to hand-computed values; the rate average excludes zero-duration rows (no divide-by-zero) while still counting their tons. Filter values are bound params (no injection). **Documented 1k-session benchmark: list+aggregate+filter+trend in ~10 ms** (functional-only in CI, never timing-gated, §4.2). Self-review (arch+design+red-team): no BLOCKING (id≈start-order ordering, commodity = "refined" semantics, and the ended-only history scope all documented + sound). ~24 analytics tests; core suite 355 green; full test + typecheck + lint + compliance green.
- 2026-07-13 — **Overlay arrange — movable + resizable + pinnable (operator request, before Phase 3).** The overlay gained a LOCK model: **locked** (default) = the click-through, display-only HUD (today's behaviour, safe in flight); **unlocked (arrange)** = the window becomes interactive with a drag bar (`-webkit-app-region: drag`, native move) + grabbable resize edges (`setResizable(true)`), and the renderer draws a dashed border + drag bar. Driven from the Command Deck's new **Arrange/Lock** button (new `overlay.lock` invoke) and a **Ctrl+Shift+L** global shortcut; the mode is pushed to the overlay over WS (new `overlay.mode` channel, included in the onConnect primer) so it stays **WS-only** — the overlay renderer still has zero IPC. Placement (bounds) persists across launches via a small `overlay-state.json` store (`main/overlay-state-store.ts`); the lock state is deliberately NOT persisted — **the overlay always boots LOCKED** so it can never restart into a click-blocking state (red-team fix). `setLocked` maps to `setIgnoreMouseEvents`/`setMovable`/`setResizable`/`setFocusable`; `showInactive` still means it never steals game focus. ~15 new tests (overlay-state fold of `overlay.mode`, OverlayApp arrange chrome, `createOverlayWindow` lock/bounds/onBoundsChanged via the mocked BrowserWindow, the bounds store, the `overlay.lock` IPC, the Command Deck Arrange button). Full suite + typecheck + lint + compliance + e2e (6/6) green. Native edge-resize + click-through arranging remain in the operator's over-game manual check (`docs/verification/phase-2.md`).
- 2026-07-13 — **Step 2.10 done — Overlay v1 (read-only, click-through). PHASE 2 (Assay) feature-complete; overlay pending the operator's over-game manual check.** A frameless, transparent, always-on-top, CLICK-THROUGH (`setIgnoreMouseEvents(true)` + renderer `pointerEvents:none`) window shows the latest MINE/SKIP verdict + cargo % over the borderless-windowed game. **New `@lodestar/overlay` package:** `OverlayApp`/`VerdictHud`/`CargoStrip` (inline-styled, self-contained — no Tailwind) over a pure `overlay-state` fold + the SOLE sanctioned loopback WebSocket client (`ws/loopback-client.ts`, injectable socket factory for tests). The `WebSocket` global is re-permitted there in BOTH the eslint egress wall AND the compliance raw-socket scanner (`overlay/src/ws`), narrowly — fetch/XHR/raw sockets stay banned everywhere. **WS-only, no IPC:** the overlay's preload exposes ONLY `{port,token}` parsed from `additionalArguments` (renderer argv — never a URL query param, never logged, §5.6); there is no `ipcRenderer`, so its telemetry is provably WS-only (asserted). **Late-joiner fix:** the 1.9 WS server gained an `onConnect` hook — main primes each new client with the current `state.snapshot` (its delta baseline) + the latest verdict before any broadcast (single-threaded connection handler guarantees snapshot-first ordering); verdicts now broadcast to WS, not just IPC. **New `overlay.toggle` invoke channel** (closed-union add) drives the Command Deck toggle button; a global shortcut (Ctrl+Shift+O, registration checked + warned) also toggles; `showInactive` so the overlay never steals focus from the game. **electron-vite** gained a 2nd renderer HTML (`overlay.html`, CSP `connect-src` permits `ws://127.0.0.1:*` only — the fix that stops the loopback WS being CSP-blocked) + a 2nd preload entry; the 2-entry build was verified locally. ~40 new tests (overlay package: WS client reconnect/dispatch, state fold, HUD render; desktop: `overlayWindowOptions` + `createOverlayWindow` via a mocked BrowserWindow asserting click-through, `parseOverlayConnection`, the overlay preload's single-key exposure, the ws-server `onConnect` primer + throw-isolation, the overlay.toggle IPC + Command Deck button click). Self-review (arch+design+red-team): no BLOCKING — dependency-direction clean (overlay→shared+react), the token is never logged, click-through is belt+braces, the late-joiner baseline converges (full-key deltas), `onConnect` throw is isolated; MINOR fixed: global-shortcut registration is now checked + logged if unavailable. Green: full test suite (node ABI, coverage thresholds met) + typecheck + eslint + prettier + banned-patterns + dependency-direction + compliance (raw-socket scan skips the sanctioned WS dir).
- 2026-07-13 — **Assay page restyled to the deck language + `ScreenHeader` adopted app-wide (operator: "use this as the new default style").** Extracted the Command Deck's header (eyebrow + title + hairline, optional trailing status) into a reusable `components/ScreenHeader.tsx` and switched both the Command Deck and the Assay screen to it (the deck's local `DeckHeader` deleted). Rebuilt `screens/Assay.tsx` on the same surface: centered `max-w-6xl` container, a 4-up glass KPI strip (Prospected / Hit rate / Avg best / Motherlodes) with the deck's stat idiom, and the deck's 8/4 grid rhythm (`VerdictCard` hero + `h-full` Recent-prospects panel). Same data + testids (`assay-screen`, `hit-rate-strip`, `assay-empty`, verdict/history contracts) — all Assay tests unchanged and green. The two screens now read as one designed surface; `ScreenHeader` is the default chrome for every future module screen. Green: renderer 96/96 + typecheck + eslint + prettier + banned-patterns + dependency-direction.
- 2026-07-13 — **Command Deck redesign (operator UX request — not a new step; Step 1.10 file list edited).** The deck read as six equal, ragged tiles with fuel shown twice at two precisions and no cargo-capacity context. Reorganized into a purposeful hero bento with real hierarchy: `SituationPanel` (activity + location + status-flag chips, folding the former Activity + Location tiles) and `CargoPanel` (a fill gauge vs ship capacity — count / cap t + %, omitted when capacity is unknown) lead as the hero row; `SessionStatsPanel` (Tons/hr + Credits/hr as rate heroes over sub-stats) and `FuelPips` (retitled **Fuel & Power** — now the deck's ONLY fuel readout) sit beneath; `VesselStrip` (slim inline ship-identity footer, no fuel) trails. `{ShipPanel,LocationPanel,ActivityPanel}.tsx` deleted (folded away, no dead code). Centered `max-w-6xl`, glass/theme tokens reused, ONLINE badge gained a "Live" pulse. Same live telemetry + first-class GAME OFFLINE / not-configured states. `CommandDeck.test.tsx` updated to the new DOM (all behavioral coverage kept) + new assertions locking the cargo-fill gauge (5/256 → 2%; absent when capacity unknown) and single fuel readout (`29.89 t` shown exactly once). Self-review (arch+design+red-team): no BLOCKING — cargo % guards div-by-zero/negative + clamps 0–100, `place` guards empty-string ring/body, docked-with-no-station falls back, renderer→shared-types-only dependency direction intact. Green: renderer 96/96 + typecheck + eslint + prettier + banned-patterns + dependency-direction.
- 2026-07-13 — **Step 2.9 done — Assay UI panel (the dashboard).** The visual payoff: `apps/desktop/src/renderer/screens/Assay.tsx` — a live animated MINE (orange, glowing) / SKIP (dim) `VerdictCard` (Framer Motion, keyed on prospectId), the structured `ReasonList` (rendered verbatim from each reason's code+fields), rock-composition bars, a `ProspectHistory` (last-12, newest-first), a live hit-rate strip (prospected / hit-rate / avg-best / motherlodes from the 2.8 `session.stats.prospectStats`), and the pre-first-prospect empty state ("fire a prospector limpet"). **New plumbing:** the orchestrator's emitted verdict became the SHARED `AssayVerdictEvent` (carries composition + content + remaining; `reasons` widen soundly from the Layer-1 `Reason` union to a flat wire `AssayReason`, no cast); the main pushes it on a new `assay.verdict` channel; the renderer accumulates verdicts in an app-level Zustand store (`useAssayStore`, wired in App.tsx so history survives screen switches). Assay module flipped `available`. **Real-clock latency test:** 200 parsed-prospect→verdict-delivered legs through the REAL bus→orchestrator→verdict-engine→SQLite path timed with `performance.now()` (no injected clock) — p95 ≤ 150ms. ~25 tests (assay-format, store, screen incl. empty state + verdict render + hit-rate strip, latency). Review (arch+design+red-team) — **PASS, no BLOCKING/MINOR**: the Reason→AssayReason widening confirmed a genuine subtype relation (sound under exactOptionalPropertyTypes); single app-level subscriber (no double-record/leak); latency test exercises the real path (not trivial). NITs (all unreachable from the trusted producer: the shared `subscribe` null-allowance, per-prospect duplicate-material-name key) noted, no action. Green: lint + typecheck + test + compliance.
- 2026-07-13 — **Step 2.8 done — prospector statistics.** `packages/core/src/session/prospect-stats.ts` `computeProspectStats(entries) → ProspectStats`: a PURE fold over a session's prospects + verdicts into hit rate (MINE ÷ prospected), average best-material % (over rocks that HAD materials — a material-less rock doesn't dilute it), per-commodity distribution (keyed by the canonical dominant material, 2.2), and motherlode count. Derived — the persisted prospects are the source of truth, so stats are recomputable (no new migration). `ProspectStats` added to `SessionSummary` (shared). **Live streaming:** the state bridge gained `enrichSession` (decorates every `session.stats` payload) + `touchSession()`; the Assay wiring calls `touchSession()` on each verdict so fresh stats stream even though a prospect isn't an engine state/session change; the enricher recomputes from the displayed session's prospects at send time. Golden fixture (5 prospects → hitRate 0.6, avg 21.4, motherlode 2, byCommodity {painite:3,platinum:1,rutile:1}) pins exact hand-computed values. Review (arch+design+red-team) — **1 BLOCKING fixed:** the enricher keyed on `engine.sessionId()`, which goes undefined at session end while `session()` stays non-null → stats zeroed out at the review-your-haul moment; added `engine.lastSessionId()` (tracks the last persisted session, survives session end) + extracted a tested `enrichSessionStats` helper (ended-session regression test). MINOR fixed: `avgBestMaterialPct` divisor. NITs noted (per-tick re-query is O(n) but index-backed + few-Hz — acceptable; unknown-commodity fallback keying). Green: lint + typecheck + test + compliance.
- 2026-07-13 — **2.7b follow-up — voice picker + LibriTTS option (operator feedback: Ryan sounded robotic).** Pinned a second voice `en_US-libritts-high` (warmer LibriTTS; verified SHA-256 `328e3e9c…`, GitHub-sourced like Ryan) and turned the Settings voice field into a **picker** (new read-only `tts.voices` invoke channel → `VOICE_CATALOG` from the manifest; the app downloads whichever voice is selected, hash-verified). Operator auditioned both samples and chose "both + picker". Noted honestly: the genuinely-newer Piper voices (Jenny, libritts_r, hfc) are HuggingFace-Xet-only, which the hardened downloader can't fetch, so they're not viable; a real naturalness jump needs a different LOCAL engine (Kokoro-82M) — a candidate for a future dedicated step. Green: lint + typecheck + test + compliance.
- 2026-07-13 — **Step 2.7b-ii done — TTS service + audio playback + Settings + wire-to-verdicts. STEP 2.7 (Piper TTS) COMPLETE; 2.6's deferred engine→bus wiring closed.** The Assay engine now SPEAKS. `apps/desktop/src/main/tts-service.ts`: synthesizes a callout WAV (via the pinned Piper, hash-verified download through the integrations downloader + the new sanctioned `nodeFetch` transport) and pushes it base64 to the renderer; lazy install collapses concurrent first-run downloads + caps re-downloads after repeated failures. `voice/src/tts/callout.ts`: verdict → spoken line ("Painite, twenty-four percent. Mine."); only MINE speaks (SKIPs silent). `apps/desktop/src/main/assay-wiring.ts` (closes 2.6): the live engine gained `onEvent` (parsed journal events) + `sessionId()`, and the wiring maps ProspectedAsteroid/MiningRefined/AsteroidCracked→the Assay bus + MarketSell→the price book, so real prospects run the verdict engine and the verdict is spoken. `renderer/audio/tts-player.ts` + `use-tts-audio.ts`: Web Audio playback, volume-clamped, decode-failure-safe. Settings **Voice (TTS)** section: enable toggle, voice display, volume slider (persists on release), **Test-phrase button** → `tts.test` IPC → synth + push + play. Channels `tts.test`/`tts.audio` + `ttsEnabled`/`ttsVoice`/`ttsVolume` settings (validated 0–1 volume). Method defaults to laser (loadout-inference is a later step). ~50 new tests (callout, tts-service incl. throw-safety + failure-cap, assay-wiring DB integration, tts-player, Settings TTS panel). Review (arch+design+red-team, security-focused) — **1 BLOCKING fixed:** `onVerdict` did `void speak().then()` with no `.catch` while `ensureInstalled` can THROW (fs EPERM/ENOSPC) → an unhandled main-process rejection per callout; now `ensure()` is throw-safe + `onVerdict` has a defensive `.catch` (throw-path test added). MINOR fixed: volume slider now persists on release not every drag tick; install re-download capped after 3 consecutive failures. NITs noted: keying the install collapse by voiceId (when a voice picker lands) + serializing overlapping callouts via the 2.7a speech queue (deferred — MINEs are seconds apart). Also fixed a latent compliance bug: the raw-fetch scanner's `skipPaths` had a stale `packages/` prefix that never matched (exposed by the first real `fetch()` in `node-fetch.ts`) → segment-anchored. Manual audio check documented in `docs/verification/tts.md` (operator to perform). Green: lint + typecheck + test + compliance.
- 2026-07-13 — **Step 2.7b-i done — verified Piper download + extraction + synthesis.** Step 2.7b split into 2.7b-i (this: pure/testable download+extract+synth + a real-binary integration test) and 2.7b-ii (TTS service + audio + Settings + manual audio), same 1.9/1.9a pattern. **Downloader (integrations):** real GitHub/HF release assets 302 to signed, expiring CDN URLs that can't be pinned, so `downloadArtifact` now FOLLOWS redirects with a per-hop INSTALL-allowlist re-check (fails closed off-allowlist, https-only via `guardUrl`, bounded hops) — the committed SHA-256 on the FINAL bytes is the integrity guarantee regardless of CDN; added `release-assets.githubusercontent.com` + `cas-bridge.xethub.hf.co` to the install allowlist (compliance snapshot updated). **Provenance (voice):** `piper-assets.ts` pins the Piper Windows binary (`piper_windows_amd64.zip`, sha256 `f3c58906…`) + the operator-chosen `en_US-ryan-high` voice (`voice-en-us-ryan-high.tar.gz`, sha256 `de346b05…`), BOTH from `github.com/rhasspy/piper` releases (HF's Xet CDN 403s a plain GET; GitHub is reliable) — closes the 0.10 committed-manifest note. `archive.ts`: pure-JS zip + tar.gz extraction (`fflate` + a minimal USTAR reader, no shell-out). `piper.ts`: `ensureInstalled` (download+extract on first run, hash-verified, injected fetch/fs) + `synthesize` (one-shot text→WAV, validates RIFF) + `createNodeRunPiper` (injectable child-process adapter). 29 voice tests + 13 downloader tests, ≥90% line/≥85% branch. **The real Piper binary + ryan-high voice were downloaded, hash-verified, and synthesized a real 108 KB WAV of "Platinum, thirty-two percent. Mine." (RTF 0.36) — the integration test runs that in-suite (gated on the binary; skipped in CI).** Green: lint + typecheck + test + compliance. **2.7b-ii (audio playback + Settings + wire-to-verdicts + the operator's manual audio check) remains.**
- 2026-07-13 — **Step 2.7a done — speech queue + shared sidecar supervisor.** Step 2.7 split into 2.7a (pure infrastructure) + 2.7b (real Piper binary + wiring + Settings + manual audio), same pattern as 1.9/1.9a — 2.7 is large and 2.7b needs external binary provenance + hardware-audio verification. Scaffolds **`@lodestar/voice`** (feature pkg → `core`|`shared`; the `ai`→`voice` firewall keeps the LLM out of speech/input). `voice/src/tts/speech-queue.ts` `createSpeechQueue({ speaker, supersede?, onError? })`: a priority queue over classes `ops-echo > safety > alert > verdict > ambient` — plays one at a time via an injected async `Speaker`, a higher class jumps ahead of lower-class BACKLOG (the current short utterance always finishes — no barge-in), identical non-supersede utterances DEDUPE (vs pending AND the playing one), and a supersede class (`verdict`) cancels its own un-spoken backlog when a newer callout arrives (new rock → only the latest matters). `core/src/sidecar/supervisor.ts` `createSupervisor({ spec, spawn, … })`: owns one long-running child — spawn, crash-restart with exponential backoff (500ms→30s clamp) + crash-loop protection (>`maxRestarts` within `windowMs` → `failed`), ordered shutdown (SIGTERM→grace→SIGKILL); all process I/O behind an injected `SpawnSidecar` so a scripted test double drives the whole state machine (reused by ML/STT in Phases 6–7). 21 tests (10 queue: 5-class preemption, FIFO-in-class, dedupe, supersede, clear, sync+async speaker errors; 11 supervisor: spawn/idempotent-start/crash-restart/crash-loop-fail/window-eviction/graceful+forced shutdown/backoff-cancel/default-curve/concurrent-stop) — 100% line, ≥94% branch both files. Review (arch+design+red-team) — **PASS, no BLOCKING**; fixed 3 MINOR: **(1)** a SYNCHRONOUS throw from `speak` (a dead-pipe EPIPE writing to the Piper child) would escape the pump and wedge all speech → now try/caught into the same recovery (regression test); **(2)** a concurrent `stop()` orphaned the first promise + leaked a grace timer → returns the in-flight promise; **(3)** the production default backoff curve was untested → direct curve test + a default-wiring test. NITs: stale-exit guard on a superseded handle, header doc tightened, `failed`-is-recoverable-via-`start()` documented. Green: lint + typecheck + test + compliance.
- 2026-07-13 — **Step 2.6 done — Assay orchestrator (live pipeline).** The runtime glue that makes Assay real: `packages/core/src/assay/orchestrator.ts` `createAssayOrchestrator({ bus, prospects, overrides, priceBook, speak? })` subscribes to a typed `EventBus<AssayEvents>` — on `prospected` it saves the observation, merges overrides over the pure defaults (`mergeThresholds`), runs the pure `assay` (Layer 1, `@lodestar/intelligence`) with the live 2.5 price book, persists `verdict` + structured `reasoning` JSON onto the prospect row (new repo `saveVerdict`), publishes the `verdict` event on the bus, and hands the callout to the `speak` hook (Piper 2.7 subscribes there). **Acted-on correlation** (journals carry NO asteroid identity, so it's TEMPORAL): a MINE opens a window on the just-prospected rock (eligible = the called commodities); a `refined` (canonical id) of one, or a `cracked`, while open marks the prospect acted-on (`markActedOn`); a deep-core `cracked` also flags that exact open rock cracked (`markCracked`); the window closes when the NEXT prospect supersedes it — no wall clock, deterministic. `prospect-repository` surfaces `verdict`/`reasoning`/`actedOn`. **Never breaks the bus:** the bus DETACHES a throwing subscriber, so every handler is fully try/catch-isolated (a bad prospect logs + the next still assays — FK-throw tested). 26 tests on the REAL bus + a temp DB (prospect→verdict persist+emit with correct reasons; live value/t via the price book; mined-after-MINE; ignored-SKIP; different-commodity/different-session non-correlation; window-closes-at-next-prospect; deep-core crack; live override flips MINE→SKIP; DB-failure isolation) — orchestrator.ts 100% line / 90% branch. Review (arch+design+red-team) — **no BLOCKING**; fixed 2 MINOR: **(1)** `onCracked` flagged `cracked` via the old `markLastCracked(sessionId)` "most-recent" heuristic, which after an intervening SKIP would pollute the SKIP rock → replaced with a precise `markCracked(open.id)` on the open MINE rock (repo `markLastCracked` → `markCracked(id)`; regression test added); **(2)** a mid-handler DB failure after `save()` left the superseded window open (misattribution) → the window now closes IMMEDIATELY after `save()` and re-opens only on MINE success. NIT: commented that the refine path correlates only canonical-resolvable commodities. **Divergence (per the SSOT Files list = orchestrator.ts only):** the `verdict` event is published on the bus; the `assay.verdict` IPC/WS channel + the engine→bus wiring land with their renderer consumer (the Assay UI panel, 2.9) — the orchestrator is complete + fully tested but not yet fed by the live engine. Green: lint + typecheck + test + compliance.
- 2026-07-13 — **Step 2.5 done — price book v1 (migration 004).** The Assay verdict's value/t source. Migration 004 (`market_snapshots`, §5.5) as an inlined SQL const (001 pattern): commodity sell prices per market, keyed by CANONICAL commodity id (2.2), with `source`/`source_ts`/`station_name`/`star_system`, `UNIQUE (commodity_id, market_id, source)` (upsert = latest observation wins), designed so Phase-4 galaxy sources widen it ADDITIVELY (new `source` values, ADD COLUMN — never a rebuild). `packages/core/src/market/price-book.ts` `createPriceBookStore(db)`: `ingestMarket(snapshot)` (on-dock `Market.json`, skips `sellPrice ≤ 0` = station-doesn't-buy + unknown commodities, atomic transaction), `ingestSale(MarketSell)`, `best(id)` (highest sell price across markets, latest-ts tiebreak, age-stamped), and `resolver()` → the `(id) → price | undefined` the pure 2.4 `assay` consumes. **Cross-source join proven:** a `Market.json` `$painite_name;` symbol and a `ProspectedAsteroid` "Painite" both canonicalize through `commodityFromInternal` to the same id, so docking updates the price the verdict reads (integration test: value/t 0 → 150k after a 500k×30% ingest). `core → intelligence` now a real dep (allowed; the 2.6 orchestrator wires it into product code next). 7 price-book tests + full data/core suites green. Review (arch+design+red-team) — **PASS, no BLOCKING** (SQL upsert/best-query/units/atomicity/source-mixing all verified sound; source-mixing can only move the value/t number, never the MINE/SKIP call, which is proportion-driven): actioned the one MINOR — replaced the redundant `idx_market_commodity` (the UNIQUE auto-index already covers `commodity_id` lookups) with a composite `idx_market_best (commodity_id, sell_price DESC, source_ts DESC)` that also serves the `best()` sort. NITs (Phase-4 `PriceSource` widening + surfacing `starSystem` on `BestPrice`) deferred, already documented in the migration header. Green: lint + typecheck + test + compliance.
- 2026-07-13 — **Step 2.4 done — MINE/SKIP verdict engine.** The signature feature: `intelligence/src/assay/verdict.ts` `assay(prospect, method, thresholds, priceBook) → Verdict` — PURE, deterministic. `Verdict = {call, score, reasons}`; reasons are a STRUCTURED union (`motherlode`/`already-depleted`/`proportion-above-threshold`/`price-weighted-value/t`/`content-tier`) so the UI and TTS render each verbatim. Material names are canonicalized via `commodityFromInternal` (2.2); `score` = price-weighted value/t (Σ price×proportion/100). **Precedence pinned:** depleted (`remainingPct ≤ 0`) → SKIP BEATS motherlode → MINE; else any material at/above its commodity×method threshold → MINE, else SKIP. 29 table-driven tests (100% line, boundary at/just-below threshold, wrong-method exclusion, unthresholded/unknown commodity, negative-remaining, empty rock, every reason code). Review (precedence + math verified sound): **1 BLOCKING fixed** — the SSOT says "dominant VALUE" but the multi-material sort used PROPORTION (would make TTS speak Platinum 34%/68k over Painite 28%/140k) → now sorts by price-weighted contribution (proportion as tiebreaker for the no-price case), test corrected. NOTEs: SSOT signature reordered to `(prospect, method, thresholds, priceBook)` to match the code; added wrong-method / unthresholded / negative-remaining / empty-rock tests. Green: lint + typecheck + intelligence tests (CI runs the full node-ABI suite).
- 2026-07-13 — **Step 2.3 done — commodity × method threshold matrix.** Scaffolds the pure **`@lodestar/intelligence`** package (Layer 1; depends on `shared` only — dependency-direction-clean). `intelligence/src/assay/thresholds.ts`: `DEFAULT_THRESHOLDS` — the worth-mining matrix as `{commodityId, method, minProportion, note}` entries, exactly the SSOT-listed laser(8)/deep-core(11)/subsurface(5) pairs, each with a provenance note; `defaultThreshold`, `mergeThresholds` (override wins; tuple-encoded key), `MOTHERLODE_ALWAYS_MINE`. **Compile-time id safety:** added `COMMODITY_IDS`/`CommodityId` union to `shared/commodities.ts` and typed `ThresholdEntry.commodityId: CommodityId`, so a typo'd matrix id fails `tsc` (a consistency test pins COMMODITIES↔COMMODITY_IDS both ways). `core/src/settings/threshold-overrides.ts`: validated (known commodity, valid method, 0–100 %) per-commodity×method overrides persisted as JSON in the `settings` kv; merge is the 2.6 orchestrator's job (core does NOT import intelligence). 20 tests (intelligence 100% coverage). Review (matrix cross-checked entry-by-entry vs the SSOT text — correct): **1 BLOCKING fixed** — `read()` only shape-checked tampered rows, so a semantically-invalid override (unknown commodity / out-of-range %) survived → now re-runs the semantic validator on read (+ tests for tampered semantic rows, null elements, non-array). NOTEs: exact-SSOT-matrix pin test, tuple-encoded merge key. (Reviewer's "drop node types" suggestion reverted — intelligence type-checks shared's `URL`-using source, so `@types/node` is required; purity stays enforced by the import scanner.) Green: lint + test + compliance.
- 2026-07-13 — **Step 2.2 done — canonical commodity dictionary.** `packages/shared/src/commodities.ts`: THE commodity-identity module every Assay feature joins through. **Canonical id = journal internal name lowercased** (stable, unambiguous). `COMMODITIES` table (20 mineables) with internalName / `$symbol;` / displayName / eddnName / inaraName / methods. `commodityFromInternal` resolves ALL internal-scheme names (`ProspectedAsteroid.Name`, `MiningRefined.Type`, `MarketSell.Type`, `Market.json` symbol — strip `$..._name;`, trim, lowercase); `commodityFromEddn` maps EDDN display names; unknown → typed `{ok:false, reason:"unknown-commodity"}` (never a silent miss). 71 shared tests (100% line/func): one fixture PER SCHEME → same id for Void Opals/LTDs/Platinum/Painite/Rhodplumsite/Tritium + Osmium/Bromellite/Alexandrite, case+whitespace tolerance, unknown path, index-collision-free, and EXACT per-commodity method sets. Review (arch+design+red-team; the reviewer cross-checked every internal name/symbol against the authoritative EDCD `commodity.csv` — all correct): **1 BLOCKING fixed** — Platinum + Painite were missing the `subsurface` method (contradicting the 2.3 matrix); the methods test used `arrayContaining` and couldn't catch it → rewrote to exact `Set` equality for every 2.3 commodity; also added `.trim()` to `internalKey` + fixed a no-op trim test, dropped redundant eddn/inara overrides, corrected a misleading Tritium comment. Green: lint + test + compliance.
- 2026-07-13 — **Step 2.1 done — prospect event capture + persistence (migration 003).** Phase 2 (Assay) opens. `packages/core/src/journal/events/prospected-asteroid.ts`: the `Prospect` observation type + `toProspect(event)` mapper (full fidelity — every material proportion, content tier, motherlode, remaining %). Migration 003 (`prospects`, §5.5) as an inlined SQL const (001 pattern), FK to `sessions` (nullable — a prospect can happen with no active session), `verdict`/`reasoning`/`acted_on` nullable for the Assay engine (2.4/2.8). `packages/core/src/session/prospect-repository.ts`: `save`/`listBySession`/`listRecent` + `markLastCracked` — a deep-core crack flags the session's MOST RECENT prospect (a TEMPORAL heuristic, NOT an identity claim; journals carry no asteroid identity, so every prospect is an independent observation). 9 tests: all-fields round-trip, partially-depleted (<100) as its own row with `remaining_pct`, no-motherlode, null-session, FK-enforced, cross-session crack isolation, and corrupt-materials surfaced (not silently dropped). Review (arch+design+red-team): no BLOCKING; fixes — `parseMaterials` now throws on a non-array shape (surface corruption, consistent no-silent ethos), added the FK-rejection + cross-session-isolation tests, documented the crack heuristic (2.6 narrows it to deep-core content), and corrected the Step-2.1 file list (no `asteroid-cracked.ts`; parsing was done in 1.5). Green: lint + test + compliance.
